...
def CO2_Temp_Hum(): #CO2¡¢ÎÂÊª¶Èº¯Êý
    import serial
    import binascii
    ...
    try:
        ...
        print("It's the problem of sensor")
        logging.inf("It's the problem of sensor")
    ser.close()

def Water_Tem(a):  #Ë®ÎÂº¯Êý
    import serial
    import binascii
    from time import sleep
    ser = serial.Serial('/dev/ttyAMA0',9600,timeout =0.5)
...
def Light_intensity(b):     #¹âÕÕÇ¿¶Èº¯Êý
    import serial
    import binascii
    from time import sleep
    ser = serial.Serial('/dev/ttyAMA0',9600,timeout =0.5)
    ...
    try:
        if c==1:
            ...
            while len(rec)==7 and rec[:1]=='\x01':
                ...
                Water_lec=float(water_le)*3/2000
               #print(Water_lec)
                break
        elif c==2:
            ser.write("")
...
'''
CO2,Temp,Humi=CO2_Temp_Hum()           #CO2¡¢ÎÂÊª¶Èµ÷ÓÃº¯Êý   
Light=Light_intensity()
a=input("Enter value from 1 to 4:")   
#Water_T=Water_Tem(a)                  #Ë®ÎÂµ÷ÓÃº¯Êý 
b=input("Enter value from 1 to 4:")
#Light=Light_intensity(b)              #¹âÕÕÇ¿¶Èµ÷ÓÃº¯Êý
#c=input("Enter value from 1 to 4:")
#Water_L=Water_lev(a)                  #Ë®Î»µ÷ÓÃº¯Êý
'''